...
exports.getPeritonrumMemo = async function (req, res) {
    ...
    try {
        ...
        const hemodialysisMemos = await dialysisDao.getPeritonrumMemo(id, year, month);
...
exports.changePeritonrumMemo = async function (req, res) {
    const {file, body: {dialysis, dialysisId}, verifiedToken: {id},} = req;
    ...
    try {
        ...
        const [isSuccess, message, imageUrl] = await dialysisDao.updateGenaraldialysisMemo({
            imageUrl: (file && file.location) ? file.location : null,
            dialysis,
            dialysisId
        });
