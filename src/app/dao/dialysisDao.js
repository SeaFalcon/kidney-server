...
exports.insertHemodialysisMemo = async function ({
  imageUrl,
  recordDate,
  memo,
  userId,
  ...
  try {
    ...
    if (isExistdialysisHeaderRows.length)
      throw new Error("이미 투석일지 / 메모가 작성된 날입니다.");

    const insertDialysisHeaderQuery = `
        INSERT INTO DialysisHeader (userId, recordDate, dialysisTypeId) 
        VALUES (?, ?, ?);
      `;
    const insertDialysisHeaderParams = [
      userId,
      recordDate,
      dialysisTypes.HEMODIALYSIS,
    ];
    ...
    await connection.query(
      insertDialysisDetailQuery,
      insertDialysisDetailParams
    );
...
exports.getHemodialysisMemo = async function (userId, year, month) {
  ...
  const getHemodialysisHeaderMemoParams = [
    userId,
    year,
    month,
    dialysisTypes.HEMODIALYSIS,
  ];
  const [getHemodialysisHeaderMemoRows] = await connection.query(
    getHemodialysisHeaderMemoQuery,
    getHemodialysisHeaderMemoParams
  );
  ...
  if (getHemodialysisHeaderMemoRows.length) {
    for (dialysisMemo of getHemodialysisHeaderMemoRows) {
      ...
      const [getHemodialysisDetailMemoRows] = await connection.query(
        getHemodialysisDetailMemoQuery,
        getHemodialysisDetailMemoParams
      );
      ...
      if (getHemodialysisDetailMemoRows.length) {
        const {
          ...
          memo,
          photo,
        } = getHemodialysisDetailMemoRows[0];
...
exports.updateHemodialysisMemo = async function ({
  imageUrl,
  memo,
  dialysisId,
}) {
  ...
  try {
    ...
    const [getImageDialysisDetailRows] = await connection.query(
      getImageDialysisDetailQuery,
      getImageDialysisDetailParams
    );
    ...
    const updateDialysisDetailParams = imageUrl
      ? [memo, imageUrl, dialysisId]
      : [memo, dialysisId];
    await connection.query(
      updateDialysisDetailQuery,
      updateDialysisDetailParams
    );
    ...
    return [
      true,
      "혈액투석 메모 수정에 성공했습니다.",
      getImageDialysisDetailRows ? getImageDialysisDetailRows[0].photo : null,
    ];
  } catch (err) {
    console.log("err", err);
    connection.release();
...
exports.deleteHemodialysisMemo = async function (dialysisId) {
  ...
  try {
    ...
    await connection.query(
      deleteDialysisHeaderQuery,
      deleteDialysisHeaderParams
    );
    ...
    await connection.query(
      deleteDialysisDetailQuery,
      deleteDialysisDetailParams
    );
...
exports.insertGeneraldialysisMemo = async function ({
  imageUrl,
  recordDate,
  dialysisType,
  dialysis,
  ...
  try {
    ...
    const insertDialysisHeaderParams = [
      ...
      recordDate,
      dialysisType === 1
        ? dialysisTypes.MACHINE_DIALYSIS
        : dialysisTypes.GENETAL_DIALYSIS,
    ];
    const [insertDialysisHeaderResult] = await connection.query(
      insertDialysisHeaderQuery,
      insertDialysisHeaderParams
    ...
    logger.error(
      `App - insertGeneralDialysisMemo Query error\n: ${err.message}`
    );
...
exports.updateGenaraldialysisMemo = async function ({
  imageUrl,
  dialysis,
  dialysisId,
}) {
  ...
  try {
    ...
    logger.error(
      `App - updateGenaraldialysisMemo Query error\n: ${err.message}`
    );
